...
### Минимальный уровень
  ...
  4. Описать возможные проблемы при масштабировании проекта:
     - Отсутствие адекватной архитектуры приводит к тому, что файл main.py становится хаотичной коллекцией методов, что затрудняет понимание его структуры и функциональности;
     - Необходимо надежно проверять наличие указанного города, используя методы, отличные от сервисов погоды;
     - Неоптимизированный запрос на получение списка городов из-за обращения к API погоды по каждому городу в списке;
     - При множестве запросов некоторые запросы могут дублироваться. В данном случае можно было бы задуматься о кешировании данных;
     - SQLite удобна для проектирования, однако при использовании в реальном проекте потребуется администрирование, включая систему пользователей и управление их правами.
